...
document.addEventListener("DOMContentLoaded", () => {
  const form = document.getElementById("formEmpleadoAdmin");
  ...
  const inputsEventos = [
    ...
    { id: "rolEmpleado", evt: "change", fn: () => validarSelect("rolEmpleado", "un rol o cargo") },
  ...
  if (form) {
    form.addEventListener("submit", (evento) => {
      ...
      const rolValido = validarSelect("rolEmpleado", "un rol o cargo");
      ...
      if (indexExistente !== -1) {
        usuariosGuardados[indexExistente] = empleadoGuardar;
        alert("Empleado actualizado con éxito.");
      } else {
        usuariosGuardados.push(empleadoGuardar);
  ...
  if (btnEliminar) {
    btnEliminar.addEventListener("click", () => {
      ...
      if (confirm(`¿Estás seguro de que deseas eliminar al empleado con RUN "${runClean}"?`)) {
